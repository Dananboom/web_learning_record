...
1. 左浮动盒子的 `left outer edge` 可能不是 `containing block` 的 `left edge`。类似的规则适用于右浮动元素。
...
#### 9.5.2 Controlling flow next to floats: the ['clear'](https://www.w3.org/TR/2011/REC-CSS2-20110607/visuren.html#propdef-clear) property

**'clear'**

| *Value:*          | none \| left \| right \| both \| [inherit](https://www.w3.org/TR/2011/REC-CSS2-20110607/cascade.html#value-def-inherit) |
| ----------------- | ------------------------------------------------------------ |
| *Initial:*        | none                                                         |
...
### [值](https://developer.mozilla.org/zh-CN/docs/Web/CSS/clear#values)
...
非 `none` 值的 `clear` 属性可能会导致 `clearance`。`clearance` 会阻止边距折叠，并且充当元素 `margin-top` 上方的空间。它用于将元素在垂直方向跨过浮动元素。
...
2. 否则，要么父元素没有参与边距折叠，要么只有父元素的下边距参与折叠。该元素的 `top border` 边界与该元素的 `bottom border` 非 `0` 时一样。

> 注：发生折叠贯穿（`collapse through`）的元素的位置不会影响其他参与边距折叠的元素的位置；只有在布局这些元素的后代时才需要确定元素的 `top border` 边界。

其实就是因为clear以后，浮动后面的盒子外边距顶端必须低于浮动盒子的底部，从之前没清除浮动的位置到清除浮动后的距离就是所谓的`clearance`。

#### 9.6.1 fixed positioning

固定定位是绝对定位的子类。唯一不同的是固定定位的 `containing block` 是 `viewport`。**对 [continuous media](https://www.w3.org/TR/CSS22/media.html#continuous-media-group)，固定定位盒子不随文档的滚动而移动。**这方面类似于 [fixed background images](https://www.w3.org/TR/CSS22/colors.html#background-properties)。对于 [paged media](https://www.w3.org/TR/CSS22/page.html)，固定定位的盒子每一页都会重复。例如，这对于在每个页面的底部放置一个签名很有用。固定定位的盒子在初始包含块中不可见的部分将不会打印。
...
1. 如果 `display` 属性为 `none`，那么 `position` 和 `float` 属性无效。这种情况，元素也不会生成盒子。
2. 如果 `position` 属性值为 `absolute` 或 `fixed`，盒子是绝对定位的，盒子 `float` 的 `computed value` 为 `none`，`display` 设置如下表。盒子的位置由 `top, left, right, bottom` 属性和 `containing block` 决定。
3. 如果 `float` 属性值不为 `none`，盒子是浮动的，并且 `display` 属性值设置如下表。
4. 如果元素是根元素，`display` 设置如下表，除了在 CSS 2.2 中未定义的指定的 `list-item` 是否会变成 `block` 或 `list-item` 计算值。
...
#### Specifying the stack level: the ['z-index'](https://www.w3.org/TR/CSS22/visuren.html#propdef-z-index) property
...
根元素形成根堆叠上下文。其它的堆叠上下文由 `z-index` 属性不为 `auto` 的定位元素（包括相对定位）生成。堆栈上下文不一定与包含块有关。未来版本的 CSS，其他属性也可能会创建堆叠上下文，例如'[opacity](http://www.w3.org/TR/css3-color/#transparency)' 属性。
...
4. 非定位的浮动元素
5. in-flow 的行内非定位的后代元素，包含 `inline-table` 和 `inline-block`。
...
绘制顺序将递归运用在每一个堆叠上下文上。堆叠上下文的绘制顺序在 [Appendix E](https://www.w3.org/TR/CSS22/zindex.html) 中有详细描述。